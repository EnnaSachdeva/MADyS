...
class RoverDomainHeterogeneous:
    ...
    def __init__(self, args, reward_type_idx, type, env_id):
        ...
        self.rover_closest_poi = [self.args.dim_x*2 for _ in range(self.args.num_agents)] # distance of all rovers (trucks+uavs) from POIs
        self.rover_farthest_poi = [-self.args.dim_x * 2  for _ in range(self.args.num_agents)] # distamce to all rovers from POIs (UAVs + Trucks)
        self.uav_closest_rover = [self.args.dim_x * 2 for _ in range(self.args.num_agents)] # distance of all uavs (trucks+uavs) from trucks
    ...
    def reset(self):
        ...
        self.rover_closest_poi = [self.args.dim_x * 2 for _ in range(self.args.num_agents)]  # distance of POI from all rovers (UAVs + Trucks)
        self.uav_closest_rover = [self.args.dim_x * 2 for _ in range(self.args.num_agents)]  # distance of all trucks from UAVs
        self.rover_farthest_poi = [-self.args.dim_x * 2 for _ in range(self.args.num_agents)] # distamce to all rovers from POIs (UAVs + Trucks)
        self.uav_closest_uav = [self.args.dim_x * 2 for _ in range(self.args.num_agents)]    # distance of all uavs from a UAV
        self.rover_closest_uav = [self.args.dim_x * 2 for _ in range(self.args.num_agents)]  # distance of trucks from UAVs
        self.rover_closest_rover = [self.args.dim_x * 2 for _ in range(self.args.num_agents)]# distance of fire trucks from other fire trucks
        self.rover_closest_poi_2 = [self.args.dim_x * 2 for _ in range(self.args.num_agents)]  # distance of POI from all rovers (UAVs + Trucks)
    ...
    def get_joint_state(self):
        ...
        for rover_id in range(self.args.num_agents): # for each rover, check each POI and other rovers, whether that POI is in that range
            ...
            poi_id = -1
            for loc, status, value in zip(self.poi_pos, self.poi_status, self.poi_value): # for each poi
                #if status == 0 and self.reward_type_idx == -1: continue #If accessed ignore (only for global tasks)
                poi_id += 1
                if status == 0: continue #If accessed ignore (only for global tasks), joint state should be same
        ...
        if self.args.is_proxim_rew:
            for i in range(self.args.num_agents):
                ...
                if (agent_type == 0):  # for UAV
                    ...
                    ## go to nearest poi
                    if (self.rover_closest_poi[i] <= self.args.obs_radius[agent_type]):  # POI is outside the range of truck
                    ...
                    if self.args.reward_scheme == 'multiple':
                        ...
                        ### go away from nearest poi
                        agent_proxim_reward.append(-proxim_rew)  # go away from nearest poi

                        ## go to farthest poi
                        if self.rover_farthest_poi[i] <= self.args.obs_radius[agent_type]:
                            proxim_rew = self.args.act_dist / self.rover_farthest_poi[i]
                        else:
                            proxim_rew = 0.0
                        ...
                        if (self.uav_closest_uav[i] <= self.args.obs_radius[agent_type]):  # for UAV to be close to truck
                            proxim_rew = self.args.act_dist / self.uav_closest_uav[i]  # fixme: this self.args.act_dist should not be there
                        else: proxim_rew = 0.0

                        if proxim_rew > 1.0: proxim_rew = 1.0
                        agent_proxim_reward.append(proxim_rew)
                        agent_proxim_reward.append(-proxim_rew)  # go away from nearest uav

                        ## go to truck
                        if self.args.num_agent_types == 2:
                            if (self.uav_closest_rover[i] <= self.args.obs_radius[agent_type]):  # for UAV to be close to truck
                                proxim_rew = self.args.act_dist / self.uav_closest_rover[i]  # fixme: this self.args.act_dist should not be there
                            else: proxim_rew = 0.0

                            if proxim_rew > 1.0: proxim_rew = 1.0
                            agent_proxim_reward.append(proxim_rew)
                            agent_proxim_reward.append(-proxim_rew)  # go away from uav
