...
class SSNE:
	def __init__(self, args):
		...
		self.popn_size = args.policy_selector_popn_size
		self.crossover_prob = args.crossover_prob
		self.mutation_prob = args.mutation_prob
		self.extinction_prob = args.extinction_prob
		self.extinction_magnitude = args.extinction_magnitude
	...
	def crossover_inplace(self, gene1, gene2):
		...
		for key in keys1:
			...
			if len(W1.shape) == 2:
				...
				try: num_cross_overs = random.randint(0, int(num_variables*0.3))
				...
				num_variables = W1.shape[0]
				# Cross over operation [indexed by row]
				for i in range(1):
					receiver_choice = random.random()
					if receiver_choice < 0.5:
						ind_cr = random.randint(0, W1.shape[0]-1)
	...
	def mutate_inplace(self, gene):
		...
		for i, param in enumerate(gene.parameters()): # mutate each param
			...
			if len(W.shape) == 2: # just weights, not bias
				num_weights = W.shape[0] * W.shape[1]
				ssne_prob = ssne_probabilities[i]

				if random.random() < ssne_prob:
					num_mutations = random.randint(0, int(math.ceil(num_mutation_frac * num_weights)))# number of mutations instances

					for _ in range(num_mutations):
						int_dim1 =  random.randint(0, W.shape[0]-1)
						int_dim2 = random.randint(0,W.shape[-1]-1)
						random_num = random.random()

						if random_num < super_mut_prob:
							W[int_dim1, int_dim2] += random.gauss(0, super_mut_strenght * W[int_dim1, int_dim2] )
						elif random_num < reset_prob:
							W[int_dim1, int_dim2] = random.gauss(0, 0.1)
						else:
							W[int_dim1, int_dim2] += random.gauss(0, mut_strength * W[int_dim1, int_dim2])

						# Regularization hard limit
						W[int_dim1, int_dim2] = self.regularize_weight(W[int_dim1, int_dim2], self.weight_clamp)
				...
				if random.random() < ssne_prob:
					...
					for _ in range(num_mutations):
						...
						if random_num < super_mut_prob:
							W[ind_dim] += random.gauss(0, super_mut_strenght * W[ind_dim])
	...
	'''
	def rouletter_wheel(self, probs, num_samples):
	...
	def evolve(self, pop, net_inds, fitness_evals, migration, states):
		...
		if isinstance(fitness_evals[0], list):
			for i in range(len(fitness_evals)):
				if self.ccea_reduction == "mean": fitness_evals[i] = sum(fitness_evals[i])/len(fitness_evals)
				elif self.ccea_reduction == "lineancy": fitness_evals[i] = max(fitness_evals[i])
